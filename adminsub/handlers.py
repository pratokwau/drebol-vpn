...
# ── Заведение подписок в панели ───────────────────────────────────────────────

async def push_subs_to_panel(progress=None) -> dict:
    """Заводит все админские подписки в панели и складывает новые ссылки.

    Кто уже есть — не дублируется, а приводится к нашим данным: срок, лимит
    устройств, трафик и сквады. Запускать можно сколько угодно раз, например
    после чистой установки панели или восстановления базы из бэкапа.
    """
    import asyncio
    from adminsub.storage import all_subs_for_panel
    from panel import create_client

    rows = await all_subs_for_panel()
    done, changed, errors = 0, 0, []
    for i, row in enumerate(rows, 1):
        sub_id, tg_id, email, expire, limit_hwid, total_gb, old_url = row
        try:
            from database import get_user_info
            user_row = await get_user_info(tg_id) if tg_id else None
            uname = user_row[2] if user_row and user_row[2] else None
            res = await create_client(
                expire_date=expire,
                limit_ip=0,
                limit_hwid=int(limit_hwid or 0),
                total_gb=int(total_gb or 0),
                email=email,
                tg_id=tg_id,
                note=f"@{uname}" if uname else "",
            )
            if not res.get("success"):
                errors.append(f"{email}: {res.get('error')}")
                continue
            link = res.get("sub_url") or ""
            if link:
                await update_sub_field(sub_id, "sub_url", link)
                changed += link != old_url
            if res.get("sub_id"):
                await update_sub_field(sub_id, "sub_id", res["sub_id"])
            if res.get("uuid"):
                await update_sub_field(sub_id, "uuid", res["uuid"])
            done += 1
        except Exception as ex:
            errors.append(f"{email}: {type(ex).__name__}: {ex}")

        if progress and (i % 10 == 0 or i == len(rows)):
            await progress(i, len(rows))
        # панель просит паузу между запросами при пачке
        await asyncio.sleep(0.08)
    return {"ok": True, "total": len(rows), "done": done,
            "links_changed": changed, "errors": errors}


async def handle_subs_push(query, context=None):
    ...
    rows = await all_subs_for_panel()
    if not rows:
        await query.answer("Заводить некого: админских подписок нет", show_alert=True)
        return
    ...
    warn = ("\n⚠️ <b>Сквады не выбраны</b> — люди заведутся без доступа.\n"
            if not squads else "")
    await query.edit_message_text(
        "🚚 <b>Завести админские подписки в панели</b>\n\n"
        f"<blockquote>Подписок: <b>{len(rows)}</b>\n"
        f"Сквадов: <b>{len(squads)}</b></blockquote>\n"
        f"{warn}\n"
        "Срок, лимит устройств и трафик возьму из базы. Кто уже есть в панели — "
        "обновлю, а не продублирую. Просроченные заведу тоже: панель сама "
        "посчитает их истёкшими.\n\n"
        "<i>Ссылки в боте обновятся сразу. Людям их передай сам — "
        "рассылки для админских подписок нет.</i>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🚚 Завести", callback_data="subs_push_go")],
            [InlineKeyboardButton("◀️ Отмена", callback_data="admin_subs")],
        ]),
    )


async def handle_subs_push_go(query, context):
    from html import escape
    from telegram import InlineKeyboardButton, InlineKeyboardMarkup
    ...
    res = await push_subs_to_panel(progress)
    errors = res.get("errors") or []
    lines = ["✅ <b>Готово</b>", "",
             "<blockquote>"
             f"Подписок: <b>{res['total']}</b>\n"
             f"Заведено и обновлено: <b>{res['done']}</b>\n"
             f"Сменились ссылки: <b>{res['links_changed']}</b>\n"
    ...
    if res["links_changed"]:
        lines += ["", "<i>У кого ссылка сменилась — отправь новую: она видна "
                  "в карточке подписки.</i>"]

    from log_channel import send_log
    await send_log(context.bot,
                   f"🚚 Админские подписки в панели: обработано {res['done']}, "
                   f"ссылок сменилось {res['links_changed']}, ошибок {len(errors)}")
    await query.edit_message_text(
        "\n".join(lines), parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("◀️ К подпискам", callback_data="admin_subs")]]),
        disable_web_page_preview=True,
    )
